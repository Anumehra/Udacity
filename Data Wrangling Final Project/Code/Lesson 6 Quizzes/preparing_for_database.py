...
def islower(attr):
    return bool(re.compile(r'^([a-z]|_)*$').search(attr))
     
def islower_colon(attr):
    return bool(re.compile(r'^([a-z]|_)*:([a-z]|_)*$').search(attr))
...
def shape_element(element):
    ...
    for child_element in list(element):
        ...
        if child_element.tag == 'tag':
            ...
            elif (islower_colon(tag_key) or islower(tag_key)):
...
def test():
    ...
    # data = process_map('san-francisco_california.osm', True)
